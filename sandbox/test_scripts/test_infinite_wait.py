...
async def test_infinite_wait():
    ...
    try:
        ...
        # 创建输出目录
        output_dir = Path(__file__).parent / "infinite_wait_test"
        ...
        if "antispider" in current_url or "验证码" in title or "captcha" in content.lower():
            ...
            print(f"   ♾️ 程序将一直等待直到验证完成...")
            print(f"   💡 提示：按 Ctrl+C 可以中断程序")
            ...
            if verification_result["success"]:
                ...
                print(f"   ❌ 等待人工验证失败")
                print(f"   等待时间: {verification_result.get('wait_time', 0)}秒")
                print(f"   最终URL: {verification_result.get('current_url', 'N/A')}")
                print(f"   最终标题: {verification_result.get('title', 'N/A')}")
        else:
            print(f"\n5. 未检测到验证码，直接尝试下载...")
